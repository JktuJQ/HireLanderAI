...
class Evaluator:
    ...
    def generate_questions(self, criteria, livecoding=False):
        ...
        if livecoding:
            promt = """Ты — ассистент, который генерирует одно задание для лайв-кодинга (написание кода в реальном времени) для собеседования на указанную должность.

                Входные данные тебе даются в формате:
                - Название должности: <строка>
                - Список критериев — массив пунктов, где каждый пункт содержит:
                  - сам критерий (короткое название)
                  - почему предыдущая LLM выбрала этот критерий, соотнесённо с текстом вакансии (пара предложений)
                  - ссылка на текст описания вакансии

                Требования к выходу:
                1. Сгенерируй **ТОЛЬКО** текст задания — ни одной лишней строки, комментария или пояснения.
                2. Задание должно напрямую проверять указанные критерии. Если один приоритетный критерий нельзя полноценно проверить в одном упражнении, разбей задание на 2–3 подзадачи, ясно пометив их внутри текста задания.
                3. Включи в задание:
                   - краткое описание задачи (чего ожидают от кандидата),
                   - технические ограничения и требования (язык/стек; если язык не указан — выбери наиболее подходящий и укажи его),
                   - входные/выходные форматы или примеры (минимум 1 пример ввода/вывода),
                   - критерии приёма работы (чёткие вещи, которые проверяют успешность — корректность, производительность, тесты, обработка ошибок и т. п.),
                   - ориентировочное ограничение по времени и по объёму кода (например, 45–60 минут; не более N строк) — если в описании вакансии есть указания по уровню (junior/mid/senior), подбери адекватное время и сложность.
                4. Язык задания — русский (если в вакансии явно указан язык интервью — используй его).
                5. Стиль — конкретный и исполняемый: кандидат должен понимать, что нужно реализовать прямо сейчас в среде для лайв-кодинга.
                6. Никакой служебной информации: **вывод должен содержать только само задание** (можно с пунктами/подзадачами внутри), без заголовков типа "Задание:" или "Ответ:" и без метаданных.
                """
        else:
